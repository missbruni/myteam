...
const Textfield: React.FC<TextfieldProps> = ({ required, label, type, errorText }) => {
  return (
    <div className="relative h-16 min-w-[200px] mb-8">
      <input
        ...
        placeholder={label}
        required={required}
        className="peer h-full w-full border-transparent border-b border-white bg-transparent pt-2 pb-1.5 pl-4 outline outline-0 transition-all focus:border-emerald-300 text-white placeholder:text-gray-500 focus:outline-0 disabled:border-gray-500 invalid:[&:not(:placeholder-shown)]:border-rose-400"
      />
      <span
  ...
  after:peer-[:not(:placeholder-shown):invalid]:border-rose-400"
      />
      {errorText && (
        <p className="mt-1 invisible  text-rose-400 text-[12px] italic peer-[:not(:placeholder-shown):invalid]:visible">
